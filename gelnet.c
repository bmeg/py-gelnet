		     ...
		     double* w, double* b, int* bSilentp, int* bNonneg )
{
  ...
  // Working storage
  double* work_a1 = (double*) calloc( n, sizeof( double ) );
  
  if( !(*bSilentp) )
    fprintf(stderr, "Running linear regression optimization with L1 = %f, L2 = %f\n",
	     *lambda1p, *lambda2p );
  ...
  if( !(*bSilentp) )
    fprintf(stderr, "f = %f after iteration %d\n", f, iter );
  free(work_a1);
}
			...
			int* bSilentp, int* bBalanced, int* bNonneg )
{
  ...
  if( !(*bSilentp) )
    fprintf(stderr, "Running logistic regression optimization with L1 = %f, L2 = %f\n",
	     *lambda1p, *lambda2p );
